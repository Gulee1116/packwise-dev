...
- Runtime dump validation and online connector status expose cross-section consistency errors for duplicate IDs, recipe item references, tag entry counts, tag registry references, quest dependencies, completed quest refs, quest item refs, and player stage refs; connector status also reports declared/uploaded/missing sections and `upload_complete` for partial-upload diagnosis.
