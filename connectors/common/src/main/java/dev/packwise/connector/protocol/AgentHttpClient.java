...
public final class AgentHttpClient {
    ...
    public String sendRuntimeDumpManifest(RuntimeDumpManifest manifest) throws IOException, InterruptedException {
        return postJson("/v1/connectors/" + pathSegment(manifest.connectorId()) + "/runtime-dumps", manifest.toJson());
    }

    public String sendAsk(QueryAsk ask) throws IOException, InterruptedException {
        return postJson("/v1/query/ask", ask.toJson());
    }
    ...
    public String sendRuntimeDumpSection(
        ...
        HttpRequest request = HttpRequest.newBuilder(resolve(
                        "/v1/connectors/"
                                + pathSegment(connectorId)
                                + "/runtime-dumps/"
                                + pathSegment(dumpId)
                                + "/sections/"
                                + pathSegment(sectionName)))
