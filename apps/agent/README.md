...
- `packwise_agent.runtime_index`：轻量 runtime section 索引，支持 `mods`、registries、tags、recipes、advancements、`ftb_quests`、player/team progress、stages NDJSON。
- `packwise_agent.runtime_dump_importer`：把本地 `packwise-dumps/<dump_id>/` 按 connector HTTP 上传相同的 service handler 导入内存态 agent。
- `recipes` section 可包含可选 `ingredient_items`，用于基础材料缺口回答。
...
如果服务器 `/packwise dump` 已经通过 `PACKWISE_AGENT_URL` 上传到正在运行的
agent，可追加 `--agent-url "<agent-url>"`。该选项会把在线 agent 的
